...
// équipe et réglages/billing). Le super-admin plateforme (platform_admin, ex « admin »)
// est traité owner sur le compte consulté et ne peut être créé/édité que par
// un autre super-admin. Le rôle « operator » a été retiré du produit.
//
...
func (a *API) handleTeamCreate(w http.ResponseWriter, r *http.Request) {
	...
	if req.Role != model.RoleManager && req.Role != model.RoleOwner {
...
func (a *API) handleTeamUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if req.Role != nil && *req.Role != model.RoleManager && *req.Role != model.RoleOwner {
		writeErr(w, http.StatusBadRequest, "Rôle invalide (manager ou owner)")
		return
	}
