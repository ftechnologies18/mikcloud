...
func portalBrandingFingerprint(db *model.DB, router *model.Router) string {
	...
	parts := []string{
		...
		t.PortalTicker,   // N°138 — bandeau animé sous le logo : servi dans la config, donc dans la sig
		t.PortalWhatsapp, // N°139 — support WhatsApp du footer : servi dans la config ET les marqueurs, donc dans la sig
		t.PortalKey,
